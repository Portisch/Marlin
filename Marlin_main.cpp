...
inline void gcode_M109() {
  ...
  /**
   * M191: Rxxx Ramp bed down with x°C/min
   *       Txxx Target temperature of bed
   */
  inline void gcode_M191() {
    if (DEBUGGING(DRYRUN)) return;  
    
    LCD_MESSAGEPGM(MSG_BED_RAMP);
    
    float ramp = code_seen('R') ? code_value_temp_abs() : 5.0;
    float theTarget = code_seen('T') ? code_value_temp_abs() : 40.0;
       
    // get actual temperature of bed  
    float actual_target_temp = thermalManager.degBed();
    ...
    do {
        now = millis();
        if (ELAPSED(now, next_temp_ms)) { //Print Temp Reading every 1 second while ramping down.
          next_temp_ms = now + 1000UL;
          
          // set actual bed temperature
          thermalManager.setTargetBed(actual_target_temp);
          
          actual_target_temp -= (ramp / 60.0);
          print_heaterstates();
          SERIAL_EOL;
        }
        ...
        idle();
        refresh_cmd_timeout(); // to prevent stepper_inactive_time from running out  
    } while (actual_target_temp > theTarget); 
    ...
    case 'M': switch (codenum) {
      ...
      case 109: // M109: Wait for temperature
        ...
        case 190: // M190: Wait for bed heater to reach target
          gcode_M190();
          break;
        case 191: // M191: Ramp bed temperature down
        gcode_M191();
        break;          
      #endif // HAS_TEMP_BED
